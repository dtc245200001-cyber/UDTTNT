"""
Test suite cho module Hien Vat API.
"""
...
def test_visitor_create_hien_vat_returns_403():
    """Token visitor gọi POST hien-vat → 403 Forbidden."""
    ...
    try:
        ...
        response = client.post("/api/hien-vat/", json={"name": "Test"})
        assert response.status_code == 403
    finally:
        app.dependency_overrides.pop(get_current_user, None)


def test_admin_create_hien_vat_returns_201():
    """Admin gọi POST hien-vat → 201 Created."""
    async def override_get_current_user():
        return ADMIN_USER
    ...
    with patch("routers.hien_vat.supabase_admin") as patched_sb:
        patched_sb.table = MagicMock(return_value=mock_table)

        try:
            client = TestClient(app)
            response = client.post("/api/hien-vat/", json={"name": "Hiện vật Test"})
            assert response.status_code == 201
            assert response.json()["name"] == "Hiện vật Test"
        finally:
            app.dependency_overrides.pop(get_current_user, None)
