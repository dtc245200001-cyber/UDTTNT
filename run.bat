...
start "Backend_Server" cmd /c "cd /d "%~dp0backend" && python -m uvicorn main:app --host 127.0.0.1 --port 8000 --reload"

echo [3/4] Dang khoi dong may chu Frontend Dev Server (port 5173)...
start "Museum_Frontend_Server" cmd /c "cd /d "%~dp0" && npm run dev"
