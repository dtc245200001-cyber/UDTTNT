...
export const VisitorHeader = () => {
  ...
  return (
    <header className="bg-white/95 backdrop-blur-md border-b border-amber-900/10 sticky top-0 z-40 shadow-[0_2px_15px_-3px_rgba(92,44,22,0.05)] font-sans transition-all">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 h-[76px] flex items-center justify-between gap-2 lg:gap-4">
        
        {/* 1. KHU VỰC LOGO BÊN TRÁI (~25-28%) */}
        <div className="flex items-center min-w-[230px] lg:w-[26%] shrink-0">
          <Link to="/" className="flex items-center gap-3 group">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-museum-brown to-museum-brown-dk group-hover:shadow-md flex items-center justify-center text-museum-gold border border-museum-gold/30 shadow-xs transition-all duration-300 group-hover:scale-105 shrink-0">
              <Landmark className="w-5 h-5 text-museum-gold" />
            </div>
            <div className="flex flex-col justify-center">
              <div className="font-black text-sm lg:text-[15px] tracking-wider text-museum-brown leading-tight group-hover:text-museum-gold transition-colors truncate">
                BẢO TÀNG QUỐC GIA VIỆT NAM
              </div>
              <div className="text-[9.5px] font-bold text-museum-gold uppercase tracking-widest mt-0.5">
        ...
        {/* 2. KHU VỰC MENU Ở GIỮA (~50-55%) */}
        <nav className="hidden lg:flex items-center justify-center flex-1 max-w-[680px] gap-1 xl:gap-1.5">
          {navLinks.map((link) => {
            ...
            const buttonContent = (
              <>
                ...
                {active && (
                  <span className="absolute bottom-1 left-3 right-3 h-0.5 bg-museum-gold rounded-full" />
                )}
              </>
            );

            const baseClasses = `relative h-9 xl:h-10 px-3 xl:px-3.5 rounded-xl text-xs xl:text-[13px] font-semibold transition-all duration-200 flex items-center justify-center gap-1.5 group select-none cursor-pointer ${
              active
        ...
        {/* 3. KHU VỰC TÀI KHOẢN BÊN PHẢI (~18-22%) */}
        <div className="hidden md:flex items-center justify-end min-w-[200px] lg:w-[24%] gap-2 shrink-0">
          {isAuthenticated ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                to="/login"
                className="h-9 xl:h-10 px-4 text-xs xl:text-[13px] font-bold text-museum-brown bg-transparent hover:bg-museum-cream/80 border border-museum-gold/50 rounded-xl transition-all duration-200 flex items-center justify-center gap-1.5 hover:-translate-y-0.5 cursor-pointer select-none"
              >
                <LogIn className="w-3.5 h-3.5 text-museum-gold" />
                <span>Đăng nhập</span>
              </Link>
              ...
              <Link
                to="/register"
                className="h-9 xl:h-10 px-4.5 text-xs xl:text-[13px] font-bold text-white bg-gradient-to-r from-museum-gold to-amber-600 hover:from-museum-gold-lt hover:to-amber-500 rounded-full shadow-xs hover:shadow-md transition-all duration-200 flex items-center justify-center gap-1.5 hover:-translate-y-0.5 cursor-pointer select-none"
              >
            ...
            {isAuthenticated ? (
              ...
              <div className="grid grid-cols-2 gap-2">
                <Link
                  to="/login"
                  onClick={() => setMobileMenuOpen(false)}
                  className="h-10 text-center text-xs font-bold text-museum-brown bg-transparent border border-museum-gold/50 rounded-xl flex items-center justify-center gap-1.5"
                >
                  <LogIn className="w-3.5 h-3.5 text-museum-gold" />
                  <span>Đăng nhập</span>
                </Link>
                <Link
                  to="/register"
                  onClick={() => setMobileMenuOpen(false)}
                  className="h-10 text-center text-xs font-bold text-white bg-gradient-to-r from-museum-gold to-amber-600 rounded-full shadow-xs flex items-center justify-center gap-1.5"
                >
